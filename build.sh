...
AMQP_VERSION=${AMQP_VERSION:-"$(curl -Is https://pecl.php.net/get/amqp | grep filename | curl -Is https://pecl.php.net/get/amqp | grep filename | sed 's/.*\([[:digit:]]\+\.[[:digit:]]\+\.[[:digit:]]\+\).*/\1/')"}
